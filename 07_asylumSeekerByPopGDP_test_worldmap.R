...
trad <- read.csv("07_refugeesByPopGDP_test_worldmap.csv", check.names = F, stringsAsFactors = F, row.names = 1)
...
if(getData) {
	...
	idw <- c('NY.GDP.PCAP.PP.CD', 'SP.POP.TOTL', 'NY.GDP.MKTP.PP.CD')
	...
	gdpAbs <- getwbData(idw[3], dat$iso2)
	...
	dat$totalParPIBAbs <- round((dat$total / gdpAbs[match(dat$iso2, gdpAbs$iso2c),'values'] ) * 10^9, 1)
	...
	data$gdpAbs <- round(gdpAbs[match(data$iso2, gdpAbs$iso2c),'values'])

	save(data, file = "data/07_refugeesMap_test.Rdata")
} else {
	load("data/07_refugeesMap_test.Rdata")
}
...
output.html <- paste("07_map_refugeesAsylum_", lang, ".html", sep ="")
data$geo <- countryTranslation(data$iso2, toupper(lang))[,2]

groups <- c("absolute count", trad['group1',lang], trad['group2',lang], "per 1 billion USD GDP")
...
popup_gdpAbs <- paste0(top,
  "GDP PPP : ", round(data$gdpAbs / 10 ^ 9, 1), " billions USD <br>",
  "Number of refugees and asylum seekers per 1 billion USD GDP PPP", ": ", "<strong>", data$totalParPIBAbs,
"</strong></p>", ifelse(lang == 'ar', "</div>", ""))
...
mn <- leaflet(data = data) %>% addTiles(urlTemplate = mb_tiles, attribution = mb_attribution) %>%
	...
	radius = ~sqrt(total) / 60, popup = popup_abs, group=groups[1]) %>%
	...
	addCircleMarkers(lng = ~lon, lat = ~lat, stroke = FALSE, fillOpacity = 0.4, fillColor = swi_rpal[2],
    radius = ~sqrt(totalParPIBAbs) /5, popup = popup_gdpAbs, group=groups[4]) %>%
	setView(21.824312, 39.074208, zoom = 3) %>%
	addLegend(position = "topright", title = trad['title',lang], opacity = 0, colors = NULL, labels = NULL) %>%
    addLayersControl(
       baseGroups = c(groups[1], groups[2], groups[3], groups[4]),
